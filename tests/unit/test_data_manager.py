...
def test_invalid_split_ratios_raise() -> None:
    with pytest.raises(ValueError):
        DataConfig(data_dir="data", splits=(0.6, 0.3, 0.3))


def test_train_val_split_empty(tmp_path: Path, random_time_series_n3: TimeSeriesSample) -> None:
    config = DataConfig(data_dir=str(tmp_path), load_if_exists=False)
...
def test_train_val_test_split_respects_ratios(
    ...
    config = DataConfig(data_dir=str(tmp_path), load_if_exists=False, splits=splits)
    manager = DataManager(config=config)
...
def test_setup_load_if_exists_missing_processed_raises(tmp_path: Path) -> None:
    ...
    config = DataConfig(data_dir=str(data_dir), load_if_exists=True)
    manager = DataManager(config=config)
...
def test_load_raw_missing_files_raises(tmp_path: Path) -> None:
    ...
    manager = DataManager(config=DataConfig(data_dir=str(data_dir), load_if_exists=False))
...
def test_save_processed_writes_all_splits(
    ...
    data_dir = tmp_path / "data"
    config = DataConfig(data_dir=str(data_dir), load_if_exists=False, batch_size=2)
...
def test_load_processed_populates_dataloaders(
    ...
    config = DataConfig(data_dir=str(data_dir_with_raw_and_processed), load_if_exists=True)
    manager = DataManager(config=config)
